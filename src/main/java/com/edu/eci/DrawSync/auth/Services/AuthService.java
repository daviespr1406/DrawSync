...
public class AuthService {
    ...
    AuthService(UserRepository userRepository, CognitoIdentityProviderClient cognitoClient, RestTemplate restTemplate,
            Request request) {
    ...
    /**
     * Creates a new user in AWS Cognito and stores their information in the local
     * database.
     * 
     * This method performs the following operations:
     * 1. Validates that the username is not already registered in the local
     * database
     ...
     * 5. Saves the user's basic information (username and email) in the local
     * database
     * 
     * @param user the authentication request containing the user's username,
     *             password, and email
     * @return AuthUserResponse containing the username, user attributes (email),
     *         and confirmation status (UNCONFIRMED)
     * @throws UserException if the username already exists
     *                       (USERNAME_ALREADY_EXISTS) or
    ...
    /**
     * Calculates the SECRET_HASH required for Cognito operations when the app
     * client has a secret.
     * 
     * The SECRET_HASH is computed as Base64(HMAC_SHA256(clientSecret, username +
     * clientId))
     * 
    ...
    /**
     * Confirms a user's registration using a confirmation code sent to their email
     * or phone.
     * 
     ...
     * 
     * This method queries the AWS Cognito service to fetch details about a specific
     * user
     * identified by their username. It communicates with the Cognito user pool
     * using the
    ...
    /**
     * Resends the sign-up confirmation code to the specified user via the
     * configured identity provider (for example, Amazon Cognito).
     * <p>
     * The delivery channel (email or SMS) and destination are determined by the
     * user pool configuration and the user's attributes.
     * This operation requires the service to be configured with a valid region and
     * client ID.
     *
     * @param username the unique username of the user to whom the confirmation code
     *                 should be re-sent; must match the value used at sign-up
     * @throws RuntimeException if the underlying identity provider rejects the
     *                          request (e.g., user not found, user already
     *                          confirmed, throttling, invalid parameters, or code
     *                          delivery failure)
     */
        ...
        var request = InitiateAuthRequest.builder()
                ...
                .authParameters(
                        Map.of(
                                ...
                                "SECRET_HASH", calculateSecretHash(clientId, clientSecret, loginrequest.getEmail())))
                .build();
